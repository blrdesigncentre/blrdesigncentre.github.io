...
const gameState = {
  ...
  teams: [
    { id: 'team-a', name: 'Team A', score: 10, completedThisRound: false },
    { id: 'team-b', name: 'Team B', score: 10, completedThisRound: false },
    { id: 'team-c', name: 'Team C', score: 10, completedThisRound: false },
    { id: 'team-d', name: 'Team D', score: 10, completedThisRound: false },
    { id: 'team-e', name: 'Team E', score: 10, completedThisRound: false }
  ],
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  function handleOrderChange(input) {
    ...
    if (isNaN(newOrder) || newOrder < 1 || newOrder > 5) {
      ...
      const currentPosition = getCurrentTeamPosition(teamIndex);
  ...
  function getCurrentTeamPosition(teamIndex) {
    return gameState.teams.findIndex(team => {
      const teamElement = document.getElementById(team.id);
      const teamItem = teamElement.closest('.team-item');
      const orderInput = teamItem.querySelector('.team-order-number');
      return parseInt(orderInput.dataset.teamIndex) === teamIndex;
    });
  }
  ...
  function updateTimerDisplay() {
    ...
    // Add blinking effect when timer reaches 0
  ...
  function updateRoundHighlight() {
    ...
    // Reset team completion states for new round
    gameState.teams.forEach(team => {
      team.completedThisRound = false;
    });
  ...
  document.querySelectorAll('.team-label').forEach((teamElement) => {
    teamElement.addEventListener('click', function() {
      ...
      if (gameState.teams[actualIndex].completedThisRound) {
        // Don't allow selecting completed teams
        return;
      }
  ...
  document.querySelectorAll('.location-bid-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      // Deduct points from selected team
      if (locationState.selectedTeamId !== null) {
        ...
        if (teamIndex !== -1) {
          ...
          console.log(`Location bid: Deducted ${locationState.selectedCluePoints} from ${gameState.teams[teamIndex].name}`);
        }
      }
  ...
  document.getElementById('location-identified').addEventListener('click', function() {
    if (locationState.selectedTeamId !== null) {
      ...
      if (teamIndex !== -1) {
        ...
        console.log(`Location identified by ${gameState.teams[teamIndex].name}! +40 points (total with clue: ${40 + locationState.selectedCluePoints})`);
      }
  ...
  document.getElementById('location-wrong').addEventListener('click', function() {
    // Save action for undo (clue deduction already applied)
    if (locationState.selectedTeamId !== null) {
      const teamIndex = getTeamIndexById(locationState.selectedTeamId);
      if (teamIndex !== -1) {
        saveAction({
        ...
        console.log(`Location wrong by ${gameState.teams[teamIndex].name}. No additional points.`);
      }
    }
  ...
  function startShuffleAnimation() {
    ...
    // After 3 seconds, select and display random card
    setTimeout(() => {
  ...
  function showFullscreenCardSelection(card) {
      ...
      <div class="shuffle-card-fullscreen-wrapper">
        <div class="shuffle-card-fullscreen">
          <div class="shuffle-card-fullscreen-content">
            <div class="shuffle-selection-announcement">This Card Got Selected!</div>
            <h3>${card.effect}</h3>
            <p>${card.detail}</p>
          </div>
        </div>
    ...
    // After 5 seconds, start return animation
    setTimeout(() => {
  ...
  function returnToGrid() {
    ...
    if (gameState.currentTeam !== null) {
      gameState.teams[gameState.currentTeam].completedThisRound = true;
      gameState.currentTeam = null; // Deselect team
      updateTeamHighlight();
      //updateTeamCompletionStates();
    }
